...
import { Image } from "react-native"; // Correct Image component
...
export default function profile() {
  ...
  // Function to check if the image exists and add a timestamp to the image URL
  ...
  useEffect(() => {
    checkImageExists().then((exists) => {
      if (exists) {
        // Add the timestamp to the image URL to force reload
        setProfileImage(imgpath + '?timestamp=' + new Date().getTime());
      } else {
        // Handle case when image does not exist (e.g., show a placeholder)
      }
    });
  }, [getMobile]); // Add getMobile as a dependency to ensure the image gets updated
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          >
            <TouchableOpacity onPress={pickImage}>
              {/* Use the correct Image component from react-native */}
              {profileImage ? (
                <Image
          ...
          {/* <Text style={styles.updatetext}>{userData.registered_date_time}</Text> */}
